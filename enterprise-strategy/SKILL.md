...
4. **Write the human to-do list to its own checklist FILE, never buried** (Action Checklist
   ...
   `docs/enterprise/strategy/action-checklist-<slug>-<YYYY-MM-DD>.md` with the standard header,
   ...
   - [ ] Set calendar: re-verify watch list on the re-check dates (or rely on the
         scheduled `--refresh` sweep, which also executes the artifact's
         `## Revisit triggers` lines as they come due)
