...
is a standalone file the user checks off as they go — never buried in an artifact footer or
left only in chat.** One shared shape across all teams so the user and the planner always know
where it is and how to read it.

**When:** any engagement that produces **3 or more** human action items writes a checklist
file. Fewer than 3 → inline checkboxes in the handoff are fine.

**Where:** `docs/enterprise/<team>/action-checklist-<slug>-<YYYY-MM-DD>.md` (`<slug>` = the
engagement, e.g. `sahlworks`, `soc2`, `q3-plan`; omit if the team has one checklist that day).
Link it from the team's `INDEX.md` like any other artifact.

**Shape (every checklist file has all of these):**

1. **Header block** — `# <Title> — Human Action Checklist`, the `Generated by` line, a
   `Status` + `Review gate` line, and `Companion to:` links to the artifacts it summarizes.
2. **`af-manual-action` marker block** (HTML comment, right under the header) so the planner
   can open a correctly-tagged issue pointing at the file:
   ```
   <!-- af-manual-action
   title: "Manual actions: <what> (post <YYYY-MM-DD> engagement)"
   labels: af-manual-action, af-manager-review
   checklist: docs/enterprise/<team>/action-checklist-<slug>-<YYYY-MM-DD>.md
   owner: founder
   -->
   ```
3. **"How to use this"** — one short paragraph: the work order, what's blocking vs. not, and a
   rough hands-on-time estimate.
4. **Ordered `- [ ]` steps** the user checks off in order — each with a **time estimate** and,
   where relevant, a **blocking-path marker** (`· gates everything below`, `**pilot-blocking**`).
   Steps a skill will do (not the human) say so and stay unchecked as pointers.
5. **The team's review-gate footer.**

**The planner** reads the `af-manual-action` marker to open one issue with `af-manual-action`
+ `af-manager-review`, titled from `title`, linking `checklist`, assigned to `owner` — the
issue body points at the file; the checkboxes live in the file, not copied into the issue.

**The printed handoff** then becomes a SHORT pointer to the checklist file ("Human to-dos are
in `<path>` — <N> steps, ~<time>; Steps <x> block <y>"), not a reprint of the list.

Reference model: `docs/enterprise/deployment/setup-checklist-*.md` and
`docs/enterprise/legal/action-checklist-*.md`.
